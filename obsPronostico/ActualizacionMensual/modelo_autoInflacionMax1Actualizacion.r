...
# para valores de parametros que produzcan errores, la documentación indica que valores usar
{
  ...
  length.fore <- strtoi(args[1])  # Num. de meses a pronosticar LISTA
  mes.shiny <- strtoi(args[2]) # mes a pronosticar
  anio.loop <- as.character(strtoi(args[3])) # ano a pronosticar
...
mes.shiny <- anio.parser[mes.shiny]

...
#####################################################################
# Programa para ejecutar todos los pronosticos y graficas DESPUES de DICIEMBRE DE 2018
...
# IMPORTANTE: mantener la estructura y nombre de archivos que se menciona en la documentacion
  ...
  monetario <- read.csv(paste(dt.file, "/ActualizacionMensual/Econ.DataMaxActualizados/Monetario.csv", sep = ""), row.names = 1)
  row.names(monetario) <- as.character(dmy(row.names(monetario)))
  costos <- read.csv(paste(dt.file, "/ActualizacionMensual/Econ.DataMaxActualizados/Costos.csv", sep = ""), row.names = 1)
  row.names(costos) <- as.character(dmy(row.names(costos)))
  demanda <- read.csv(paste(dt.file, "/ActualizacionMensual/Econ.DataMaxActualizados/Demanda.csv", sep = ""), row.names = 1)
  row.names(demanda) <- as.character(dmy(row.names(demanda)))
  precios <- read.csv(paste(dt.file, "/ActualizacionMensual/Econ.DataMaxActualizados/Precios.csv", sep = ""), row.names = 1)
...
# para valores de parametros que produzcan errores, la documentación indica que valores usar
{
  ...
  anio.parser <- c(paste0('0',1:3))  # ESTA ES LA LINEA QUE HAY QUE CAMBIAR PARA EL 2019, de moemtno solo hay datos hasta marzo 
  names(anio.parser) <- c('Enero', 'Febrero', 'Marzo')
  anio.loop <- as.character(2019) # anos despues del historico hasta diciembre de 2018
}

for( anio.shiny in anio.loop)
{
    # se itera sobre todos los anos ( de momento solo 2019 )

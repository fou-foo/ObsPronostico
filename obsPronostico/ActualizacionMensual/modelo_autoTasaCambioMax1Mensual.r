...
# Carga de librerias
{
  ...
  costos <- read.csv(paste(dt.file, "/ActualizacionMensual/Econ.DataMaxActualizados/CostosTasaDeCambio.csv", sep = ""), row.names = 1)
  ...
  precios <- read.csv(paste(dt.file, "/ActualizacionMensual/Econ.DataMaxActualizados/Tasa.csv", sep = ""), row.names = 1)
...
}
{
  ...
  anio.loop <- as.character(2019) # consideremos despues parelizar esto
  n.try <- 6         # Rezagos a probar
}
